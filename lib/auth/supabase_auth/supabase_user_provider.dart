...
class GelaFitGOSupabaseUser extends BaseAuthUser {
  GelaFitGOSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> gelaFitGOSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = GelaFitGOSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
